// The structures below are the repository for the data values extracted from
// the JSON message. The structures are populated with the extracted data by the
// "value()" member function in the main OpenWeather.cpp file.

// Some structs contain arrays so watch out for memory consumption. You can
// request a subset of the full weather report but this library grabs all values
// with one GET request to avoid exceeding the 1000 free request count per day
// (count reset at 00:00 UTC). 1000 per day means ~40 per hour. As the weather
// forecast changes slowly the example requests the forecast every 15 minutes,
// so adapting to reduce memory by requesting current, daily, hourly etc
// forecasts individually can be done.
...
// Structures for minimal set of data points for TFT_eSPI examples to reduce RAM
// needs
/*
typedef struct OW_current {
